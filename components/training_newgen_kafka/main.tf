...
module "training_newgen_kafka" {
  source = "../../modules/training_newgen_kafka"

  bastion_security_group_id = "${data.terraform_remote_state.bastion.bastion_security_group_id}"
  bastion_ip_address        = "${data.terraform_remote_state.bastion.bastion_ip_address}"
  ...
  env                       = "${var.env}"
  instance_count            = "${var.kafka["instance_count"]}"
  region                    = "${var.kafka["region"]}"
  ebs_volume_size           = "${var.kafka["ebs_volume_size"]}"
  ebs_volume_count          = "${var.kafka["ebs_volume_count"]}"
  private_key               = "${var.kafka["private_key"]}"
  cohort                    = "${var.cohort}"
  default_cohort            = "${var.default_cohort}"
}
