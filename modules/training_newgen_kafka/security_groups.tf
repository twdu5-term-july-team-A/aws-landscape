resource "aws_security_group" "kafka" {
  ...
  name        = "kafka-newgen-${var.deployment_identifier}"
...
resource "aws_security_group_rule" "kafka_emr_ingress_zookeeper_cluster_p1" {
  type                     = "ingress"
  security_group_id        = "${aws_security_group.kafka.id}"
  cidr_blocks              = ["10.0.0.0/16"]
  from_port                = 3888
  to_port                  = 3888
  protocol                 = "tcp"
  description              = "Zookeeper ingress for Zookeeper clusters"
}

resource "aws_security_group_rule" "kafka_emr_ingress_zookeeper_cluster_p2" {
  type                     = "ingress"
  security_group_id        = "${aws_security_group.kafka.id}"
  cidr_blocks              = ["10.0.0.0/16"]
  from_port                = 2888
